...
static std::string move_to_str(int move)
{
    switch (move)
    {
    case 0: // up
...
static int move_to_hm(int move)
{
    switch (move)
    {
    case 0: // up
...
static insert_result insert_tile_rand(board_t board, board_t tile)
{
    ...
    // print_board(board | tile);
    return {
        board | tile,
        std::to_string(i % 4) + "," + std::to_string(i / 4) + ".4",
    };
}
...
static board_t initial_board()
{
    ...
    return insert_tile_rand(board, draw_tile()).newboard;
}
...
//"" + tile.x + "," + tile.y + "." + tile.value; // for HAC
static std::string serialize_snapshot(std::array<int, 4 * 4> gridState, int direction, std::string added)
{
    return join(gridState, '.') + "+" + added + ";" + std::to_string(direction);
}

static std::string serialize_history(std::vector<std::string> snapshots)
{
    return "4x4S" + join(snapshots, ':');
}

static std::array<int, 4 * 4> serialize_board(board_t board)
{
    std::array<int, 4 * 4> state;
    for (int y = 0; y < 4; y++)
    {
        for (int x = 0; x < 4; x++)
        {
            uint8_t powerVal = (board)&0xf;
            board_t val = (powerVal == 0) ? 0 : 1 << powerVal;
            state[y * 4 + x] = val;
            board >>= 4;
        }
    }
...
{
    printf(serialize_history(snapshots).c_str());
    printf("\nfinal_score=%.0f\n", score_board(board));
    exit(0);
}
...
void play_game(get_move_func_t get_move)
{
    ...
    while (1)
    {
        ...
        insert_result res = insert_tile_rand(newboard, 2);
        snapshots.push_back(serialize_snapshot(serialize_board(board), move_to_hm(move), res.hstring));

        // printf("Move #%d, current score=%.0f, dir=%s\n", ++moveno, score_board(newboard), move_to_str(move).c_str());
        // print_board(newboard);
        board = res.newboard;

        if ((++moveno % 100) == 1)
        {
            printf("score=%.0f \n", score_board(newboard));
            fflush(stdout);
        }
    }
    my_handler(0);
